/*
Title: Deploying CSYCMS to Heroku
Sort: 1
*/

[Heroku](https://www.heroku.com) is a cloud hosting platform which allows you to host your code.
In this tutorial we are going to use Heroku to host our CSYCMS Knowledgebase on the [free 1 dyno plan](https://www.heroku.com/pricing).

## Prerequisites

Before we get going there are a few things we need to make sure we have in place first:

1. We are going to assume you have already [installed](%base_url%/install/installing-CSYCMS) and set up CSYCMS on your local machine
...
First you need to open a command shell in the root of your CSYCMS install. You then need to login to Heroku
...
## Clone CSYCMS and make edits

Heroku works by deploying a Git repository to their servers.  
So we need to make your CSYCMS install is a Git repository.  
...
Thankfully this is easy to setup:  
    ...
    $ git clone https://github.com/gilbitron/CSYCMS.git
    $ cd CSYCMS/
...
**NOTE: In the near future, we will isolate CSYCMS code from your documentation content (coming soon)**

## Deploy CSYCMS to Heroku
...
`git push heroku master` command after your commit changes every time you want to publish your CSYCMS site.
...
You can now visit your live CSYCMS install in the browser by running the `heroku open` command.
