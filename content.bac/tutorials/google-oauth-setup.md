...
This has been submitted as a [pull request]() on the official CSYCMS Github repository. This is my way of saying thanks to an awesome author of CSYCMS.

## Steps on how to reproduce this on fresh copy
Below are the steps one needs to take to get this working on a fresh copy of CSYCMS. In case this won't make it to the official repo, you can clone my fork [here](https://github.com/Hitman666/CSYCMS). Just make sure you set your Google OAuth credentials properly (more about this in the **X** section).

### Install packages via npm
_Make sure you first [install CSYCMS dependencies](http://docs.CSYCMS.com/install/installing-CSYCMS) after you clone it._
...
+ Add passport: `var passport=require('passport');` just after CSYCMS is required.
...
We added two scenarios for when we have Google OAuth enabled (`config.googleoauth`) and when we don't (defaulting to the current CSYCMS behavior).
...
Same thing here as well. If we have Google OAuth enabled (`config.googleoauth`) then we show the new Google login button and hide the rest. Otherwise, we default it to the current CSYCMS behavior.
