...
module.exports = function (ROOTPATH) {
  ...
  var config = {
    ...
    copyright: 'Copyright &copy; 2018 - ' + new Date().getFullYear() + ' - <a href="http://www.cseco.co.ke">CSECO</a>',
    ...
    // Which Theme to Use?
    theme_dir: path.join(ROOTPATH, '..', 'themes'),
    theme_name: 'default',

    // Specify the path of your content folder where all your '.md' files are located
    // Fix: Needs trailing slash for now!
    // Fix: Cannot be an absolute path
    content_dir: path.join(ROOTPATH, '..', 'content'),

    // Where is the public directory or document root?
    public_dir: path.join(ROOTPATH, '..', 'public'),
